...
- `sendSignal` now validates payload shape/size and rate-limits requests per user.
- `joinRoom` and `heartbeat` now enforce server-side per-user rate limits.
- Room creation now uses random 8-char room IDs with collision retries instead of counter-based deterministic IDs.
- Preview auth now requires both E2E mode and an allow-listed preview-like environment gate.
- Added persistent abuse limiter storage in schema (`abuseRateLimits`) and limiter logging.

- `sendSignal` now also rejects directed signals to non-active room members.
- Added cron-backed cleanup for stale `abuseRateLimits` buckets to limit table growth.

Revalidation note:

- On this branch, these mitigations are still relevant to the original findings and remain needed.
- Direct `develop` comparison could not be performed in this workspace because no remote tracking branch is configured.
