...
async function main() {
  ...
  gateway.onEvent(async (event, data) => {
    ...
    if (data.guild_id && data.guild_id !== config.primaryGuildId) {
      console.log(`[Worker] Ignoring event from different guild: ${data.guild_id}`);
      return;
    }
  ...
  healthServer.listen(config.port, () => {
    ...
    console.log(`[Worker] Health check endpoint: http://localhost:${config.port}/health`);
  });
