...
export class DiscordGatewayClient {
  ...
  // Event handlers
  private onEventCallback: ((event: string, data: any) => void) | null = null;
  ...
  async connect(): Promise<void> {
    ...
    const gatewayUrl = this.session.resumeUrl || 'wss://gateway.discord.gg/?v=10&encoding=json';
    ...
    try {
      ...
      switch (op) {
        ...
        case GatewayOp.InvalidSession:
          ...
          this.session = { sessionId: null, sequenceNumber: null, resumeUrl: null };
  ...
  private handleClose(code: number, reason: Buffer): void {
    console.log(`[Gateway] Connection closed (code: ${code}, reason: ${reason.toString()})`);
    ...
    if (code !== 1000 && code !== 4004 && code !== 4010 && code !== 4011 && code !== 4012 && code !== 4013 && code !== 4014) {
      this.reconnect();
    } else {
  ...
  private handleHello(data: { heartbeat_interval: number }): void {
    console.log(`[Gateway] Received HELLO (heartbeat interval: ${data.heartbeat_interval}ms)`);
