...
export function LeaveGameDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onCancel()}>
      <DialogContent className="border-border-muted bg-surface-1 sm:max-w-[400px]">
        <DialogHeader>
        ...
        <div className="flex gap-3 pt-4">
          <button
            onClick={onCancel}
            className="flex-1 cursor-pointer rounded-lg border border-border-default bg-surface-2/50 px-4 py-2.5 text-sm font-medium text-text-secondary transition-all hover:border-border-default hover:bg-surface-2 focus:outline-none focus:ring-2 focus:ring-border-default/50"
