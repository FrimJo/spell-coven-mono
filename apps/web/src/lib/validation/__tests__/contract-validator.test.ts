...
import type { MetadataFile } from '../contract-validator'
...
describe('Contract Validator', () => {
  ...
  it('should fail validation for file size mismatch', () => {
    ...
    expect(result.errors.some((e) => e.includes('Re-export embeddings'))).toBe(
      true,
    )
  })
  ...
  it('should fail validation for wrong version', () => {
    ...
    expect(result.errors.length).toBeGreaterThan(0)
    expect(
      result.errors.some((e) => e.includes('Unsupported metadata version')),
    ).toBe(true)
  ...
  it('should fail validation for wrong dtype', () => {
    ...
    expect(result.valid).toBe(false)
    expect(
      result.errors.some((e) => e.includes('Unsupported quantization dtype')),
    ).toBe(true)
  })
  ...
  it('should fail validation for wrong scale factor', () => {
    ...
    expect(result.valid).toBe(false)
    expect(result.errors.some((e) => e.includes('Invalid scale factor'))).toBe(
      true,
    )
  })
  ...
  it('should fail validation for wrong embedding dimension', () => {
    ...
    expect(result.valid).toBe(false)
    expect(
      result.errors.some((e) => e.includes('Invalid embedding dimension')),
    ).toBe(true)
