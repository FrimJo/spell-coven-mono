 ...
 */
export function createEdgeRefinementToggle(webcam: Awaited<ReturnType<typeof setupWebcam>>) {
 ...
 */
export async function refineExistingCardImage(imageUrl: string): Promise<HTMLCanvasElement | null> {
  const { loadOpenCV, refineCardEdgesWithAutoLoad } = await import('./card-edge-refiner')
